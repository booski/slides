create table if not exists `allowed_users` (`user` varchar(64) not null, primary key (`user`));
...
update `show_image` set `image` = (select `id` from `slide` where `name` = `show_image`.`image`);
